...
export function ClassPeriodRecap({ mode, classes, initialClassId, onClassChange }:{mode:"cumulative"|"matrix";classes:Array<{id:string;name:string}>;initialClassId?:string|null;onClassChange?:(classId:string)=>void}) {
  ...
  useEffect(()=>{if(initialClassId&&initialClassId!==classId)setClassId(initialClassId)},[initialClassId])// eslint-disable-line react-hooks/exhaustive-deps
  useEffect(()=>{if(classId)return; const fallback=classes[0]?.id; if(fallback)setClassId(fallback)},[classes,classId])
  const selectClass=(value:string|null)=>{if(!value)return; setClassId(value); onClassChange?.(value)}
  ...
  return <div className="space-y-4">
    <Card className="border-border/70"><CardContent className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-[220px_170px_170px_1fr_auto]">
      <div className="space-y-1.5"><Label htmlFor={`${mode}-class`}>Kelas</Label><Select value={classId} onValueChange={v=>selectClass(v)}><SelectTrigger id={`${mode}-class`} className="w-full"><SelectValue>{(value:string)=>classes.find((item)=>item.id===value)?.name??"Pilih kelas"}</SelectValue></SelectTrigger><SelectContent>{classes.map(c=><SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
