...
export function ClassPeriodRecap({ mode, classes }:{mode:"cumulative"|"matrix";classes:Array<{id:string;name:string}>}) {
  ...
  const [classId,setClassId]=useState(classes[0]?.id??""); const [from,setFrom]=useState(`${year}-${month}-01`); const [to,setTo]=useState(jakartaToday)
