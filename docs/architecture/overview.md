...
`lib/nav.ts` is the canonical navigation tree, but navigation visibility never grants access. Auth proxy and server-side guards enforce it. Administrative destinations (`/pengaturan`, `/pengaturan/pengguna`, `/pengaturan/akses`, `/pengaturan/audit`) live in the collapsible `administrasi` group of `mainNav`, filtered per permission like any other group: a group whose children are all filtered out is not rendered at all. `/development` is a top-level `mainNav` item placed immediately after the `administrasi` group — engineering documentation, not school administration, so it is deliberately not a child of that group; it requires `development.read` and renders a read-only table of the npm scripts declared in `package.json` (`lib/development-cli.ts` enriches them with category/description and is the single place that metadata lives). `accountNav` holds only self-account destinations (`/profil`); it backs the account popover at the bottom of the sidebar, which also carries the sign-out action. Administrative navigation is never moved into that popover. `SchoolSetting.timeZone` is the canonical IANA timezone for projecting real timestamps and deriving “today”; it defaults to `Asia/Jakarta` and is passed explicitly on the server or exposed through `SchoolTimeZoneProvider` on the client. Business date-only values never depend on that timezone and use `lib/school-date.ts`.
