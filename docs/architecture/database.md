...
Binary bytes are stored directly for user photos, favicon, app logo, and Sarpras photos; the accepted size of each is governed by the upload policy (`SchoolSetting` global defaults plus `UploadPolicyOverride` per slot, see [Upload architecture](uploads.md)) and applies only at upload time, never to stored rows. BOS monetary values are `Decimal(14,2)`.
