...
- **Current condition:** Migration `20260909100000_use_date_for_business_dates` aborts against production data. Verified on the local production clone: 151 `AttendanceDay` rows store `17:00:00` instead of midnight — WIB midnight written as UTC — all within 2026-09-06..2026-09-11. `SchoolHoliday`, `User.teachingSince`, `AdditionalDuty`, `StudentViolationPoint`, `BosEntry`, and `SarprasItem` are clean. Interpreting `17:00:00` as its WIB calendar date (`+1 day`) leaves **15** colliding `(classId, business date)` pairs, all on 2026-09-07; a naive `::date` cast would instead report 22 and shift every affected day backwards, which is why the interpretation must be timezone-aware.
- **Evidence:** `prisma/migrations/20260909100000_use_date_for_business_dates/migration.sql` (guard raises `Date-only migration aborted: 151 non-midnight legacy value(s)`); reproduce the full breakdown read-only with `npm run db:analyze-legacy-dates` (`prisma/legacy-date-analysis.sql`) against `sismepda_prodclone`; production itself remains at 16 applied migrations, 27 tables.
...
- **Direction:** The `17:00:00` → WIB calendar date mapping is settled. Of the 15 collisions, 11 are exact duplicates (every student's `status` and `note` identical across both rows) and can be deduplicated mechanically; **4 require an owner decision** because one student each has genuinely different attendance between the two rows (IX B, IX D, IX F, VII A — all on 2026-09-07). Encode the settled part as an explicit, repeatable legacy-data repair step that runs before the date-only migration; leave the 4 conflicts until the school confirms which record is authoritative. Do not modify the migration guard, do not cast `17:00:00` with a bare `::date`, and do not use `db push --force-reset`.
